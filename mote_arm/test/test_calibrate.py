...
from mote_arm import calibrate
from mote_arm.config import COUNTS_PER_REV, RAD_PER_COUNT, ArmConfig, JointSpec

SPEC = JointSpec(name="elbow_flex", id=3, min_rad=-1.0, max_rad=1.0, home_counts=2048)
...
def test_limits_sit_inside_the_measured_stops():
    ...
    lo, hi = calibrate.limits_from_sweep(sweep, home_counts=2000, margin=0.05)
...
def test_zero_margin_gives_the_stops_themselves():
    sweep = _record("j", _ramp(1000, 3000)).result()
    lo, hi = calibrate.limits_from_sweep(sweep, home_counts=2000, margin=0.0)
...
def test_wrapped_sweep_refuses_to_produce_limits():
    ...
    with pytest.raises(calibrate.CalibrationError, match="boundary"):
        calibrate.limits_from_sweep(sweep, home_counts=4070)


def test_home_outside_the_swept_range_is_refused():
    sweep = _record("j", _ramp(1000, 2000)).result()
    with pytest.raises(calibrate.CalibrationError, match="outside the swept range"):
        calibrate.limits_from_sweep(sweep, home_counts=2500)
...
def test_range_too_narrow_for_the_margin_is_refused():
    ...
    with pytest.raises(calibrate.CalibrationError, match="margin"):
        calibrate.limits_from_sweep(sweep, home_counts=2010, margin=0.05)


def test_home_too_close_to_a_stop_leaves_zero_unreachable():
    ...
    with pytest.raises(calibrate.CalibrationError, match="zero"):
        calibrate.limits_from_sweep(sweep, home_counts=2005, margin=0.05)
...
def test_calibrate_joint_defaults_home_to_mid_range():
    sweep = _record("j", _ramp(1000, 3000)).result()
    cal = calibrate.calibrate_joint(SPEC, sweep)
    assert cal.home_counts == 2000
    assert cal.home_source == "mid-range"
    assert cal.min_rad == pytest.approx(-cal.max_rad)


def test_calibrate_joint_uses_a_captured_home():
    sweep = _record("j", _ramp(1000, 3000)).result()
    cal = calibrate.calibrate_joint(SPEC, sweep, home_counts=2200)
    assert cal.home_counts == 2200
    assert cal.home_source == "captured"
...
def test_home_shift_is_the_radian_error_left_in_stored_poses():
    assert calibrate.home_shift(2048, 2048) == 0.0
    assert calibrate.home_shift(2048, 2148) == pytest.approx(-100 * RAD_PER_COUNT)
    assert calibrate.home_shift(2048, 2148, invert=True) == pytest.approx(
        100 * RAD_PER_COUNT
    )
    ...
    return ArmConfig.from_dict(
        {
            "arm": {
                ...
                "joints": [
                    ...
                    {"name": "gripper", "id": 6, "min": -0.1, "max": 0.1, "home": 2056},
                ],
            }
        }
    )
...
def test_emitted_block_says_where_home_came_from():
    cfg = _two_joint_config()
    sweep = _record("elbow_flex", _ramp(1000, 3000)).result()
    captured = calibrate.calibrate_joint(cfg.joint("elbow_flex"), sweep, 2100)
    block = calibrate.joints_block(
        list(cfg.joints), {"elbow_flex": captured}, None, "x"
    )
    # Single words, because the header is wrapped and a phrase may straddle lines.
    assert "captured" in block
    assert "mid-range" not in block
    assert "INWARD" in block

    derived = calibrate.calibrate_joint(cfg.joint("elbow_flex"), sweep)
    assert "mid-range" in calibrate.joints_block(
        list(cfg.joints), {"elbow_flex": derived}
    )


def test_zero_is_inside_every_emitted_band():
    """Whatever the sweep, a calibrated joint can always be commanded home."""
    cfg = _two_joint_config()
    for lo, hi, home in ((1000, 3000, None), (500, 2500, 1200), (2000, 3800, 3000)):
        sweep = _record("elbow_flex", _ramp(lo, hi)).result()
        cal = calibrate.calibrate_joint(cfg.joint("elbow_flex"), sweep, home)
...
def test_record_roundtrip(tmp_path):
    ...
    assert entry["home_source"] == "mid-range"
