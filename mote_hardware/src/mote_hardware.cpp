  ...
  const hardware_interface::HardwareComponentInterfaceParams & params)
{
  ...
  if (info_.joints.size() != 2) {
    RCLCPP_ERROR(rclcpp::get_logger("MoteHardware"),
  ...
  {
    RCLCPP_WARN(
      rclcpp::get_logger("MoteHardware"),
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("MoteHardware"),
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (raw_pos == -1) {
      RCLCPP_WARN(rclcpp::get_logger("MoteHardware"),
