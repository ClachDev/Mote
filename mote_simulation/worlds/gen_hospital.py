...
def zone(name, x, y, yaw=0.0):
    """A named navigation target for the task layer (mote_tasks)."""
    _zones[name] = (x, y, yaw)


def _check_zones():
    """Every zone must clear every box, so layout edits can't strand a zone."""
    for name, (zx, zy, _) in _zones.items():
...
def render_zones():
    lines = [
        ...
        "# not hand-edit this file. Named navigation targets for\n"
        "# hospital_world.sdf, placed by the generator (and checked for wall\n"
        "# clearance) so they stay valid when the layout changes.\n"
        "frame_id: map\n"
        "zones:\n"
    ]
    for name, (x, y, yaw) in _zones.items():
        lines.append(
...
def main():
    ...
    print(f"wrote {zones_out}: {len(_zones)} zones")
