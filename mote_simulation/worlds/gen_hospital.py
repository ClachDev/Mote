...
ROOM_RADIUS = 1.5  # footprint radius (m) for room-like zones (goto targets)

_walls = []  # list of (cx, cy, sx, sy)
_furniture = []  # list of (cx, cy, sx, sy)
_zones = {}  # name -> (x, y, yaw, radius|None)
...
def zone(name, x, y, yaw=0.0, radius=None):
    """A named place for the task layer (mote_tasks): a pose to navigate to
    (fetch waypoint or ``goto <name>``), plus an optional ``radius`` giving it
    an area footprint (rooms carry one; bare waypoints don't)."""
    _zones[name] = (x, y, yaw, radius)


def _check_zone_clearance():
    """Every zone pose must clear every box, so a layout edit cannot silently
    strand a zone inside geometry."""
    for name, (zx, zy, _yaw, _r) in _zones.items():
...
def build_world():
    ...
    # and facing into the room, each carrying a radius footprint. Room centres
    # come from the same _edges() the room banks use, so they track a layout
    # change.
    def _room_x(x0, x1, i, n=4):
        edges = _edges(x0, x1, n)
        return (edges[i] + edges[i + 1]) / 2

    zone("kitchen", _room_x(b0, b1, 0), acc_n_out + 1.25, math.pi / 2, ROOM_RADIUS)
    zone("pharmacy", _room_x(b0, b1, 2), acc_n_out + 1.25, math.pi / 2, ROOM_RADIUS)
    zone("laboratory", _room_x(b0, b1, 1), acc_s_out - 1.25, -math.pi / 2, ROOM_RADIUS)
    zone("ward_east", _room_x(c0, c1, 3), acc_s_out - 1.25, -math.pi / 2, ROOM_RADIUS)
...
def render_zones():
    lines = [
        ...
        "# the layout changes. Zones with a radius are room footprints (goto\n"
        "# targets); the rest are bare fetch waypoints.\n"
        "frame_id: map\n"
        "zones:\n"
    ]
    for name, (x, y, yaw, radius) in _zones.items():
        radius_part = f", radius: {round(radius, 3):g}" if radius is not None else ""
        lines.append(
            f"  {name}: {{x: {round(x, 3):g}, y: {round(y, 3):g}, "
            f"yaw: {round(yaw, 3):g}{radius_part}}}\n"
        )
    return "".join(lines)
...
def main():
    ...
    rooms = sum(1 for v in _zones.values() if v[3] is not None)
