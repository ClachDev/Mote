...
> **Done (2026-07-23): harness built and run — see [`sfm_stage0_results.md`](sfm_stage0_results.md).** `mote_perception/tools/sfm_stage0_geometry.py` measured all three quantities over the recorded `perception` bags. Result: the geometry is *sound when moving* (near-FOE parallax ~4 px at B = 0.10 m, well above the matcher floor; stamp jitter only ~1 mm of baseline error, not dominant), but the **usable-baseline duty cycle is the decisive limit** — the robot translates only 29–41 % of the time even on driving bags (26–32 % of frames reach a usable baseline; the rest is stopped or turning in place). So the kill criterion trips on *duty cycle*, not geometry: metric multi-view can't be the *primary* depth path. Revised staging: **stage A (video-depth flicker swap) — do; stage B (sparse anchors, holds while stopped) — optional supplement; stage C (learned-MVS replacement) — defer.**
